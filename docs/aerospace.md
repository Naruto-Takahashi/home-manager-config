...
| キー | 動作 |
| :---: | :--- |
| `Shift + T` | 分割方向をタイル横⇄縦に切り替え (NixOS/WSLと同じ`Shift+T`に統一．以前は`V`単体だったが，起動系キーの統一のためVivaldi起動に譲った) |
...
起動系キーはNixOS/WSLと同じく「アプリ名の頭文字」に統一しています (`N`=Neovim，`Y`=Yazi，`V`=Vivaldi)．
...
| `Y` | WezTerm内でYazi |
| `N` | WezTerm内でNeovim |
| `V` | Vivaldi (以前は`B`だったが，起動系キーの統一のため変更) |
