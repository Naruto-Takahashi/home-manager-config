...
このレイヤーが送信する**信号自体**は3ホスト共通ですが、その信号にWM側が何を割り当てるかはホストごと・時期によって独立して変わりうるため（実際にNixOS側はHyprlandの設定変更で`D`と`V`の割り当てが以下から既にずれています）、**発動するアクションの正はホストごとのドキュメントを見てください**：NixOSは[hyprland.md](hyprland.md)、macOSは[aerospace.md](aerospace.md)。
